...
> **2026-03-09 更新**：参照官方 Coze JS SDK（`coze-dev/coze-js`）默认值调整。
> 之前 `duration_seconds=86400` 实际被 Coze 限制在约 1 小时，导致 ECS 出现 401 错误。

- 私钥签名 JWT（RS256，exp=1h）→ 换取 access_token（duration_seconds=900，15 分钟）
- 内存缓存上限 10 分钟，提前 2 分钟视为过期，主动签发新 JWT
- 401 重试兜底：工作流调用返回 401 时清除缓存、重新获取 token 重试一次
