...
- `src/utils/coze-scheduler.ts` — 定时调度服务（Redis Hash 存储 + node-cron 定时执行）
...
**功能**：
- **循环任务**：配置 platform、limit、执行间隔（1-24h），自动 cron 定时执行
- **单次任务**：填写 platform、limit，点击「立即执行」
- **任务管理**：开关切换、删除、状态/结果查看、10 秒自动刷新
...
**存储**：Redis Hash `coze:scheduler:tasks`（field=taskId, value=JSON）

**环境行为**：
- `NODE_ENV=development`：不启动 cron 定时任务（仅支持手动执行）
- `NODE_ENV=production`：自动启动 cron，服务重启后从 Redis 恢复任务

**页面鉴权**：
- 页面本身 `/coze/scheduler` 免鉴权（PUBLIC_PATHS）
- API 调用需 API Key，页面首次打开弹出输入框，Key 存入 `localStorage` 自动使用
